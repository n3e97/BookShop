...
(function(w){
    ...
    var initPage=function(data){
        ...
        $("#book_banben").html(data.banben+'<span class="badge">新</span>');
    ...
    $.ajax({
     ...
     $("#sub_buy_num").on("click",function(){
         var self=this;
         var r=parseInt($(self).parent().find('input').get(0).value);
          r>0&&(r--);
         $(self).parent().find('input').get(0).value=r;
     });
    $("#add_buy_num").on('click',function(){
        var self=this;
        var r=parseInt($(self).parent().find('input').get(0).value);
        r<book_data.kucun&&(r++);
        $(self).parent().find('input').get(0).value=r;
