...
$(function () {
	...
	$(".book_item_full_screen").on("click", function () {
		...
		if (!dt) {
			...
			$(self).parent().parent().parent().velocity({
				width : width + "px",
				height : height + "px",
				left : "0px",
				top : "0px"
	...
	$(".book_item_save").on("click", function () {
		...
		$(this).parent().find(".book_item_refresh").css({
			visibility : "visible"
		});
		//to do:....
	});
	...
	$(".add_book_item_save").on("click", function () {
		...
		var data = {
			...
			book_price : obj.book_price.get(0).value,
			book_publisher : obj.book_publisher.get(0).value,
			banben : obj.book_banben.get(0).value,
			book_img : $("#set_img").find("img").attr("src"),
			book_num : obj.book_kucun.get(0).value,
	...
	$("#reset_img").on("click", function () {
		...
		$("#set_img").css({
			cursor : "pointer"
		}).html('<div>+</div>').on("click", function () {
			$("#input_img_slave").trigger("click");
		});
	});
...
function upload_img_cb(img_url) {
	var img = "<img src='" + img_url + "' style='width:100%;height:auto;'/>";
	$("#set_img").html(img);
	$("#input_img_slave").get(0).value = "";
	$("#set_img").off("click").css({
		cursor : "none"
	});
}
